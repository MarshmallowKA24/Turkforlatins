...
function setupMessageListener() {
    window.addEventListener('message', function(event) {
        ...
        if (event.data.type === 'updateCardStatus') {
        ...
        } else if (event.data.type === 'moveCardBack') {
            const { deckId, cardId } = event.data;
            console.log(`Moviendo tarjeta ${cardId} de vuelta desde deck ${deckId}`);
            moveCardBackToOriginalDeck(deckId, cardId);
        }
    });
}
...
function viewDeck(deckId) {
    ...
    try {
        ...
        const studyWindow = window.open('study.html', '_blank');
        if (!studyWindow) {
            showError('No se pudo abrir la ventana de estudio. Verifica que tu navegador permite ventanas emergentes.');
            return;
        }
        
        // Dar más tiempo para que la ventana cargue
        setTimeout(() => {
            console.log('📤 Enviando datos a la ventana de estudio...');
            const messageData = {
                type: 'init',
